...
export default async function handler(req,res){
 ...
 try{
  ...
  const b64=fs.readFileSync(path.join(process.cwd(),'downloads','TOX_SETUP_V006.exe.b64'),'utf8').trim();const buf=Buffer.from(b64,'base64');
  res.setHeader('Content-Type','application/vnd.microsoft.portable-executable');
  res.setHeader('Content-Disposition','attachment; filename="TOX_SETUP_V006.exe"');
