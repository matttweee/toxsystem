...
export default async function handler(req,res){
 ...
 try{
  ...
  const b64=fs.readFileSync(path.join(process.cwd(),'downloads','TOX_CLIENT_DEMO_V002.zip.b64'),'utf8').trim();const buf=Buffer.from(b64,'base64');
  res.setHeader('Content-Type','application/zip');res.setHeader('Content-Disposition','attachment; filename="TOX_CLIENT_DEMO_V002.zip"');res.setHeader('Content-Length',String(buf.length));res.setHeader('Cache-Control','private, no-store');res.status(200).end(buf);
